    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <Card style={{ padding: "20px 22px" }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: "10px", marginTop: "18px", fontSize: "13.5px", color: "#3a352c", cursor: "pointer" }}>
          <input type="checkbox" checked={form.emailReminders} onChange={(e) => setForm({ ...form, emailReminders: e.target.checked })} style={{ width: "17px", height: "17px", accentColor: "var(--accent, #3d6b53)" }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", alignItems: "start" }}>
        <ChipCard title="Book Shelves" note="Shelf codes used for physical placement" kind="shelf" mono values={lists.shelves} onChange={(v) => setLists({ ...lists, shelves: v })} placeholder="e.g. G-11" />
        <ChipCard title="Subject / Category" note="Classifications used across the catalog" kind="subject" values={lists.subjects} onChange={(v) => setLists({ ...lists, subjects: v })} placeholder="e.g. Poetry" />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", alignItems: "start" }}>
        <ChipCard title="Grade List" note="Grade levels available when registering students" kind="grade" values={lists.grades} onChange={(v) => setLists({ ...lists, grades: v })} placeholder="e.g. Grade 13" />
        <ChipCard title="Section List" note="Homeroom / class sections within each grade" kind="section" values={lists.sections} onChange={(v) => setLists({ ...lists, sections: v })} placeholder="e.g. Section E" />
      </div>
    </div>
  );
}

function ChipCard({ title, note, kind, values, onChange, placeholder, mono }: {
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
        {values.map((text) => (
          <span key={text} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "6px 6px 6px 12px", borderRadius: "20px", background: "var(--bg-soft, #f3edda)", border: "1px solid var(--border-input, #ddd2b8)", fontSize: "13px", color: "#3a352c", fontFamily: mono ? "'IBM Plex Mono',monospace" : "inherit" }}>
            {text}
            <button onClick={() => remove(text)} style={{ width: "18px", height: "18px", borderRadius: "50%", border: "none", background: "var(--border-card, #e4dcc6)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0 }}><Icon name="x" color="#6f6653" size={11} /></button>
          </span>
        ))}
      </div>
