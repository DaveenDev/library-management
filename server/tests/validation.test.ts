...
describe("route param validation", () => {
  it.each([
    ...
    const res = await (method === "patch"
      ? request(app).patch(path).send({ title: "x", name: "x" })
  ...
  it("400s rather than silently returning nothing for NaN ids", async () => {
    ...
    const res = await request(app).get("/api/members/abc/history");
    expect(res.status).toBe(400);
  });

  it("400s on a negative id", async () => {
    expect((await request(app).post("/api/loans/-1/renew")).status).toBe(400);
  });
});
...
describe("body validation", () => {
  ...
  it("rejects an invalid staff email", async () => {
    const res = await request(app).post("/api/users").send({ name: "N", email: "not-an-email" });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/email/i);
  });

  it("rejects out-of-range settings", async () => {
    expect((await request(app).put("/api/settings").send({ loanPeriodDays: 0 })).status).toBe(400);
    expect((await request(app).put("/api/settings").send({ gracePeriodDays: -1 })).status).toBe(400);
  });

  it("rejects an empty lookup value", async () => {
    expect((await request(app).post("/api/settings/lookups/subject").send({ value: "   " })).status).toBe(400);
  });

  it("rejects an unknown lookup kind", async () => {
    expect((await request(app).post("/api/settings/lookups/nope").send({ value: "x" })).status).toBe(400);
  });
});
...
describe("constraint errors surface as 4xx, not 500", () => {
  ...
  it("returns 409 for a duplicate staff email", async () => {
    ...
    expect((await request(app).post("/api/users").send(body)).status).toBe(201);
    expect((await request(app).post("/api/users").send(body)).status).toBe(409);
  });
});
