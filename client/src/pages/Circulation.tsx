...
export function Circulation() {
  ...
  const checkin = async () => {
    try {
      ...
      toast(r.amount > 0 ? `Returned · fine $${r.amount.toFixed(2)} (${r.days} days late)` : "Returned — no fine");
  ...
  const ret = async (l: Loan) => {
    try { const r = await api.returnLoan(l.id); toast(r.amount > 0 ? `Returned · fine $${r.amount.toFixed(2)}` : "Returned — no fine"); refresh(); }
    catch (e: any) { toast(e.message, "bad"); }
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
        <Card style={{ padding: "20px 22px" }}>
          ...
          <div style={{ ...scanBox, marginBottom: "14px" }}><Icon name="scan" color="#8a8069" size={17} /><input value={bookBarcode} onChange={(e) => setBookBarcode(e.target.value)} placeholder="Scan or type LIB-000000" style={scanInput} /></div>
          <label style={label}>Member ID</label>
          <div style={{ ...scanBox, marginBottom: "18px" }}><Icon name="scan" color="#8a8069" size={17} /><input value={memberCode} onChange={(e) => setMemberCode(e.target.value)} placeholder="Scan or type S-0000" style={scanInput} /></div>
        ...
        <Card style={{ padding: "20px 22px" }}>
          ...
          <label style={label}>Book barcode</label>
          <div style={{ ...scanBox, marginBottom: "14px" }}><Icon name="scan" color="#8a8069" size={17} /><input value={returnBarcode} onChange={(e) => setReturnBarcode(e.target.value)} placeholder="Scan or type LIB-000000" style={scanInput} /></div>
